                ...
                (item) => `
                  <tr>
                    <td>
                      ...
                      <span class="table-subtitle">${item.sku} 路 ${item.location_name}</span>
                    </td>
            ...
            (item) => `
              <article class="timeline-item">
                ...
                <div class="timeline-content">
                  <strong>${item.action_type} 路 ${item.product_name}</strong>
                  <p>${item.warehouse_name} / ${item.location_name} 路 Ref ${item.reference_type} #${item.reference_id}</p>
                  <span class="timeline-time">
                    ${Number(item.quantity_change) > 0 ? "+" : ""}${item.quantity_change} units 路 ${window.CoreInventoryUi.formatRelativeTime(item.created_at)}
                  </span>
                </div>
              </article>
            `
